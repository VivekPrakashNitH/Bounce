...
export const UniversalSystemDemo: React.FC<Props> = ({ level, onShowCode }) => {
  return (
    ...
    <div className="w-full max-w-4xl mx-auto bg-slate-900 rounded-xl p-8 border border-slate-700 shadow-2xl relative flex flex-col items-center text-center">
       
       {/* Header */}
       <div className="w-full flex justify-between items-center border-b border-slate-700 pb-4 mb-8">
          <div className="flex items-center gap-4">
             <BounceAvatar className="w-10 h-10" />
             <h3 className="text-xl font-mono text-white flex items-center gap-2">
                <Network className="text-cyan-400" /> {level.title}
             </h3>
          </div>
        ...
        <div className="w-full h-64 bg-slate-800/50 rounded-xl border border-slate-700 border-dashed flex flex-col items-center justify-center p-8 relative overflow-hidden group">
            <div className="absolute inset-0 bg-grid-white/[0.05]" />
            
            <div className="z-10 bg-slate-900 p-6 rounded-2xl border border-slate-600 shadow-2xl max-w-lg">
                <BookOpen size={48} className="text-cyan-500 mx-auto mb-4" />
                <h4 className="text-lg font-bold text-white mb-2">Concept Visualization</h4>
        ...
        <div className="mt-8 grid grid-cols-2 md:grid-cols-4 gap-4 w-full">
            {level.topics.map((t, i) => (
                <div key={i} className="bg-slate-800 border border-slate-700 p-3 rounded text-xs text-slate-300 font-mono">
                   {t}
                </div>
            ))}
        </div>
