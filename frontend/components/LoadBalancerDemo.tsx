...
export const LoadBalancerDemo: React.FC<Props> = ({ onShowCode }) => {
  ...
  return (
    <div className="w-full max-w-3xl mx-auto bg-black/40 backdrop-blur-xl rounded-2xl p-6 border border-white/10 shadow-2xl relative overflow-hidden group">
      {/* Subtle Grid */}
      ...
      <div className="relative z-10 flex flex-col gap-8">
        <div className="flex justify-between items-center border-b border-white/10 pb-4">
          ...
          <div className="flex gap-2">
            ...
            <span className="text-[10px] text-zinc-400 border border-white/10 px-2 py-1 rounded-full uppercase tracking-wide">Round Robin</span>
          </div>
        </div>

        {/* Visualization Area */}
        <div className="flex justify-between items-center h-48 px-4 relative select-none">
          ...
          <div className="flex flex-col items-center gap-2 z-20 relative">
            ...
            <button 
              onClick={sendRequest}
              className="w-14 h-14 rounded-full bg-white hover:bg-zinc-200 flex items-center justify-center text-black shadow-[0_0_20px_rgba(255,255,255,0.2)] transition-all active:scale-95 z-20"
            >
              <Globe size={24} />
            </button>
          ...
          {/* Visualization Paths */}
          <div className="absolute left-24 right-28 h-full top-0 pointer-events-none">
             {/* Static SVG Lines */}
          ...
          <div className="flex flex-col gap-6 z-20">
            {servers.map(id => (
              <div 
                key={id}
                className={`flex items-center gap-3 px-4 py-2 rounded-lg border transition-all duration-100 ${
                  activeServer === id 
              ...
              >
                ...
                <div className="flex flex-col">
                  <span className="text-[10px] font-bold uppercase tracking-wider">Server 0{id}</span>
                </div>
              </div>
            ))}
          </div>
