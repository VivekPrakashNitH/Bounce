...
export const CachingDemo: React.FC<Props> = ({ onShowCode }) => {
  ...
  const handleRequest = () => {
      ...
      setTimeout(() => {
          ...
          if (foundIdx !== -1) {
              // HIT
              setLog(`âœ… Cache HIT! Moved ${reqKey} to Most Recently Used.`);
              ...
              // MISS
              setLog(`âŒ Cache MISS. Fetching from DB...`);
              setStatus('fetching');
              ...
              setTimeout(() => {
                  ...
                  setCache(prev => {
                      ...
                      if (newCache.length >= CACHE_SIZE) {
                          setLog(`âš ï¸ Cache FULL. Evicting LRU: ${newCache[0].key}`);
       ...
       <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-0 border-b border-slate-700 pb-4 mb-4 sm:mb-8">
          <div className="flex items-center gap-3 sm:gap-4">
             ...
             <h3 className="text-sm sm:text-xl font-mono text-yellow-400 flex items-center gap-2">
                <Zap className="w-4 h-4 sm:w-5 sm:h-5" /> Level 4: LRU Caching
             </h3>
          </div>
