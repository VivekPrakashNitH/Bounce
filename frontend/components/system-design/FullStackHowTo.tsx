...
export const FullStackHowTo: React.FC<Props> = ({ onShowCode, onProgress, initialSectionIndex }) => {
    const [step, setStep] = useState(initialSectionIndex ?? 0);
    ...
    React.useEffect(() => {
        if (onProgress) {
            onProgress({ sectionIndex: step, totalSections: steps.length });
        }
    }, [step, onProgress, steps.length]);
