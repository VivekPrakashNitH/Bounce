...
export const BackendLanguagesDemo: React.FC<Props> = ({ onShowCode }) => {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto bg-slate-900 rounded-xl p-4 sm:p-6 border border-slate-700 shadow-2xl relative">
       <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-0 border-b border-slate-700 pb-4 mb-4 sm:mb-6">
          <div className="flex items-center gap-3 sm:gap-4">
             ...
             <h3 className="text-sm sm:text-xl font-mono text-white flex items-center gap-2">
                <Terminal className="text-green-400 w-4 h-4 sm:w-5 sm:h-5" /> Level 0: Backend Languages
             </h3>
          </div>
          <div className="flex gap-2">
             <span className="text-[10px] sm:text-xs text-slate-400 bg-slate-800 px-2 sm:px-3 py-1 rounded-full uppercase tracking-wider font-bold">
               {lang.tagline}
             </span>
          </div>
       </div>
       ...
       <div className="grid grid-cols-1 sm:grid-cols-12 gap-4 sm:gap-4">
          ...
          {/* Sidebar Selector */}
          <div className="col-span-1 sm:col-span-3 flex flex-row sm:flex-col gap-1 sm:gap-1 overflow-x-auto sm:overflow-x-visible border-b sm:border-b-0 sm:border-r border-slate-800 pb-3 sm:pb-0 sm:pr-4">
             <span className="text-xs font-bold text-slate-500 uppercase mb-2 hidden sm:block">Select Language</span>
             {(Object.keys(LANGUAGES) as Language[]).map((key) => {
                 ...
                 return (
                    <button
                      ...
                      className={`flex items-center gap-2 sm:gap-3 px-2 sm:px-3 py-2 rounded-lg text-xs sm:text-sm font-bold transition-all whitespace-nowrap ${selected === key ? 'bg-slate-800 text-white border-l-2 sm:border-l-4 border-blue-500' : 'text-slate-400 hover:bg-slate-800/50'}`}
                    >
                        <Icon size={14} className={selected === key ? l.color : 'text-slate-600'} />
                        {l.name}
                    </button>
                 );
             })}
          ...
          {/* Main Content Area */}
          <div className="col-span-1 sm:col-span-9 flex flex-col gap-4 overflow-y-auto custom-scrollbar sm:max-h-[500px]">
              
              {/* Header Info */}
              <div className="flex flex-col sm:flex-row items-start justify-between gap-3">
                 <div className="flex-1">
                    <h2 className={`text-xl sm:text-2xl font-black ${lang.color} mb-1`}>{lang.name}</h2>
                    <p className="text-slate-300 text-xs sm:text-sm">{lang.setup}</p>
                 </div>
                 <div className="bg-slate-950 p-3 rounded-lg border border-slate-800 w-full sm:w-auto sm:min-w-[200px]">
                    <span className="text-[10px] sm:text-xs font-bold text-slate-500 uppercase">Best Used For</span>
                    <div className="text-[10px] sm:text-xs text-white mt-1 leading-relaxed">
                       {lang.setup}
                    </div>
                 </div>
              </div>
              ...
              <div className="bg-blue-900/10 border border-blue-900/30 rounded-xl p-3 sm:p-4">
                 <h4 className="text-xs sm:text-sm font-bold text-blue-400 uppercase mb-3 flex items-center gap-2">
                    <Book size={14} /> Core Concepts Explained
                 </h4>
                 <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {lang.concepts.map((concept, idx) => (
                        <div key={idx} className="bg-slate-800/50 p-3 rounded-lg border border-slate-700/50 hover:border-blue-500/30 transition-colors">
                            ...
                            <div className="text-[11px] text-slate-400 mb-2 leading-snug">{concept.desc}</div>
                            <div className="text-[10px] text-blue-300 italic flex gap-1 items-start">
                                <Info size={10} className="flex-shrink-0 mt-0.5" /> "{concept.metaphor}"
                            </div>
                        </div>
                    ))}
                 </div>
              ...
              {/* Code Snippet */}
              <div className="bg-black rounded-xl border border-slate-800 p-3 relative overflow-hidden min-h-[120px] sm:min-h-[140px]">
                  <div className="absolute top-2 right-2 text-[10px] text-slate-600 font-mono">server.{selected}</div>
                  <pre className="text-[10px] sm:text-xs font-mono text-slate-300 overflow-auto custom-scrollbar">
                      <code>{lang.code}</code>
                  </pre>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                 <div className="bg-green-900/10 border border-green-900/30 rounded-lg p-3">
                    <span className="text-[10px] sm:text-xs font-bold text-green-400 uppercase mb-2 block">Pros</span>
                    <ul className="space-y-1">
                        {lang.pros.map((p, i) => (
                            <li key={i} className="text-[10px] sm:text-xs text-slate-300 flex items-center gap-2">
                 ...
                 <div className="bg-red-900/10 border border-red-900/30 rounded-lg p-3">
                    <span className="text-[10px] sm:text-xs font-bold text-red-400 uppercase mb-2 block">Cons</span>
                    <ul className="space-y-1">
                        {lang.cons.map((c, i) => (
                            <li key={i} className="text-[10px] sm:text-xs text-slate-300 flex items-center gap-2">
